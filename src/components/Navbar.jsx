...
const Navbar = () => {
    ...
    //token is indicate login /logout ...true show user is loged in
    //temperary  use of this token
  ...
  return (
    <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400'>
        <img onClick={()=>navigate('/')} className='w-24 h-22 cursor-pointer' src={assets.logoN6} alt="" />
        <ul className='hidden md:flex items-start gap-5 '>
            <NavLink to='/'>
                <li className='py-1 hover:text-blue-500'>Home</li>
                <hr className='border-none outline-none h-0.5 bg-primary w-3/5 m-auto hidden'/>
            </NavLink>
            <NavLink to='/doctors'>
                <li className='py-1 hover:text-blue-500'>All Doctors</li>
                <hr className='border-none outline-none h-0.5 bg-primary w-3/5 m-auto hidden'/>
            </NavLink>
            <NavLink to='/about'>
            ...
            <NavLink to='/contact'>
                <li className='py-1 hover:text-blue-500'>Contact</li>
                <hr className='border-none outline-none h-0.5 bg-primary w-3/5 m-auto hidden'/>
            </NavLink>
            <NavLink to='/login'>
                <li className='py-1 hover:text-blue-500'>Login</li>
                <hr className='border-none outline-none h-0.5 bg-primary w-3/5 m-auto hidden'/>
            </NavLink>
        </ul>
        <div className='flex items-center gap-4'>
            {
                ...
                <div className='flex items-center cursor-pointer group relative'>
                    ...
                    <div className='absolute top-0 right-0 pt-14 text-base font-normal text-gray-600 z-20 hidden group-hover:block'>
            ...
            <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
                ...
                <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
                    ...
                    <NavLink onClick={()=>setShowMenu(false)} to={'/login'}><p className=' px-4 py-2 inline-block rounded'>Login</p></NavLink>
                </ul>
            </div>
        </div>
    </div>
